...
WORKDIR /src
COPY ["validator-api.csproj", "validator-api/"]
RUN dotnet restore "validator-api/validator-api.csproj"

WORKDIR "/src/validator-api"
COPY . .
RUN dotnet build "validator-api.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "validator-api.csproj" \
...
Execute unit tests
```
    cd "validator-api/test/validator-api.Tests"
    dotnet test
```

Deploy application
```
    cd "validator-api/src/validator-api"
